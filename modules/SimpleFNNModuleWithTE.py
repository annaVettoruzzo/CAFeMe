...
class TaskEncoderComplex(torch.nn.Module):
    ...
    def cnn_block(self, in_channels, out_channels):
        return torch.nn.Sequential(
            torch.nn.Conv2d(in_channels, out_channels, 3),
            #torch.nn.BatchNorm2d(out_channels, track_running_stats=False),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(2, 2),
        )
...
class ComplexFNNModuleWithTE(torch.nn.Module):
    def __init__(self, conv_dim=[1, 32, 64, 64], dense_dim=[1024, 512, 512], n_classes=10, modulation="c1"):
        ...
        if modulation in ["c0", "c1"]:
            ...
            self.te = TaskEncoder(out_shapes=[(2, conv_dim[1], 1, 1), (2, conv_dim[2], 1, 1), (2, conv_dim[3], 1, 1), (2, dense_dim[0]), (2, dense_dim[1]), (2, dense_dim[2])])
