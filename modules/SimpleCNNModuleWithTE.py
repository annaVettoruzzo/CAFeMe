...
# -------------------------------------------------------------------
class FedModulator(torch.nn.Module):
...
# -------------------------------------------------------------------
class SimpleFedModualtor(torch.nn.Module):
...
# -------------------------------------------------------------------
class CNNModuleWithTE(torch.nn.Module):
    """
    ...
    def __init__(self, conv_dim=[3, 64, 64, 64], dense_dim=[576, 576, 576], n_classes=62, modulation="c1"):
        ...
        if modulation in ["c0", "c1"]:
            self.fm = FedModulator(conv_dim, dense_dim[0], out_shapes=[(1, conv_dim[1], 1, 1), (1, conv_dim[2], 1, 1), (1, conv_dim[3], 1, 1), (1, dense_dim[0]), (1, dense_dim[1]), (1, dense_dim[2])])
        elif modulation in ["c2"]:
            self.fm = FedModulator(conv_dim, dense_dim[0], out_shapes=[(2, conv_dim[1], 1, 1), (2, conv_dim[2], 1, 1), (2, conv_dim[3], 1, 1), (2, dense_dim[0]), (2, dense_dim[1]), (2, dense_dim[2])])
    ...
    def forward(self, x):
        r1, r2, r3, z0, z1, z2 = self.fm(x)  # output of the federated modulator
...
class SimpleCNNModuleWithTE(torch.nn.Module):
    ...
    def __init__(self, conv_dim=[1, 32, 64], dense_dim=[1024, 512], n_classes=10, modulation="c1"):
        ...
        if modulation in ["c0", "c1"]:
            self.fm = SimpleFedModualtor(out_shapes=[(1, conv_dim[1], 1, 1), (1, conv_dim[2], 1, 1), (1, dense_dim[0]), (1, dense_dim[1])])
        elif modulation in ["c2"]:
            self.fm = SimpleFedModualtor(out_shapes=[(2, conv_dim[1], 1, 1), (2, conv_dim[2], 1, 1), (2, dense_dim[0]), (2, dense_dim[1])])
    ...
    def forward(self, x):
        r1, r2, z0, z1 = self.fm(x) # output of the federated modulator
