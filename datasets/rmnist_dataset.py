...
class RotatedMNIST(Dataset):
    def __init__(self, num_clients=100, k_clusters=10):
    ...
    def __getitem__(self, index):
        img, target, group_id = self.data[index], self.targets[index], self.group_assign[np.where(self.data_indices == index)[0][0]]
        img = img / 255.0
        img = img.unsqueeze(0)
        img = transforms.functional.rotate(img, int(group_id*20)) # to have 0,20,40,60,80,..,180,200 degrees
        #img = torch.rot90(img, k=int(group_id + 1), dims=(1, 2)) # to have 0, 90, 180, 270 degrees
        return img, target
    ...
    def assign_groups(self, num_clients, k_clusters):
        ...
        assert len(chunks[0]) % clients_per_cluster == 0, "data per cluster is not divisible by clients per cluster"
